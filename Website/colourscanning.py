...
def video_frame(text_scramble):
    ...
    #returns full name of colour for display purposes
        ...
        if string_cube == scanned_cube:
            ...
            for i in range(len(colours)):
                ...
                while rotate <= 4 and not match:
                    #using move method from the Cube class on cube object
    ...
    while True:
        ...
        #sets result based on current scan
        ...
        #scanning the cube when s pressed, only allows scanning for 6 faces
        if cv2.waitKey(1) & 0xFF == ord('s') and (face_num <= 5):
            face = []
            print('current middle colour: ', M)
            print('colour it is supposed to be: ', sides[face_num])
            #only saves scan if the middle of the cube is the correct colour to be scanned
            if M == sides[face_num]:
                for i in corner:
                    current = i
                    face.append(current)
                ...
                if '' in face:
                    ...
                    face_num += 1
            #starts timing start of error and type of error for display later
            else:
        ...
        if text_showing == True:
            #displays error message based on type of error
            if error_type == 'face':
            ...
            if error_type == 'blank':
                ...
                pass
            #using start time to calculate how long text has been displayed for
            if (time.time() - error_start_time) >= 3: 
                text_showing = False 
        ...
        #turning into correct data type
...
#taken out of main subroutine so it can be imported and used in scramblemap.py
def colour_to_BGR(colour): 
        if colour == 'r':
            return (0,0,255)
        elif colour == 'o':
