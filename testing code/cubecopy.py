...
# from _scramblegen import *
#I'm importing all of the subroutines in the scramblegen file
#s = gen()
...
class Cube:
    ...
    #swapping the pieces based on the move done, this swaps one piece at a time.
...
#use copycube.py method and change views.py so instead of cube = scramble(y)
#it will become c = cube() , cube = c.scramble(y), OOP reusing code. 
    
#this scrambles thecube
def scramble(s):
    c = Cube()
    # print(s)
    # print(len(s))
    for i in range(len(s)-1):
        ...
        #checking to see if it a plain move or another variation of a move (', 2)
            if s[i][1] == '':
                # colour to be rotated = white , hence 0 is passed in
                c.clockwise_move(0)
